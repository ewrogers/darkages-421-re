...
| Table function | `0` | Produces the identity table, where generated byte `T[x]` is `x`. |
...
- `A` be the action byte.
- `D` be `payload || 0x00`.
- `K` be the repeating key with length `k`.
- `T` be the generated table.
- `S` be the current client send sequence.
...
It copies `S` to the output, increments the stored counter modulo 256, and transforms `D` in this order:
...
`net_build_xor_table` accepts function indexes 0 through 9. For input `x` from 0 through 255, it generates:

| Function | Generated byte |
|---:|---|
| 0 | `x` |
