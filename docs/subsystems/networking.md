...
Only ordinary transformed client bodies pass through `net_c_encode_packet_body`; those bodies carry the current sequence and increment the client counter modulo 256. Cleartext client bypass bodies do not contain a sequence byte and do not advance that counter.
