...
| `Darkages.exe:0x005207E0` | `ui_exit_wait_pane_vtable` | virtual table | exit-wait pane | Includes the server action `0x4C` handler used by the orderly-exit exchange. | [SReconnect](../protocol/server-messages.md#0x4c-sreconnect) | Installed by `ui_exit_wait_pane_ctor`; the source-level C++ class name is not yet established. |
